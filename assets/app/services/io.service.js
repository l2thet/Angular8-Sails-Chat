System.register(['@angular/core', 'rxjs/Subject'], function(exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __metadata = (this && this.__metadata) || function (k, v) {
        if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
    };
    var core_1, Subject_1;
    var ioService;
    return {
        setters:[
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (Subject_1_1) {
                Subject_1 = Subject_1_1;
            }],
        execute: function() {
            ioService = (function () {
                function ioService() {
                    this._ioMessage$ = new Subject_1.Subject();
                    this.registerSailsListener();
                }
                Object.defineProperty(ioService.prototype, "ioMessage$", {
                    get: function () {
                        return this._ioMessage$.asObservable();
                    },
                    enumerable: true,
                    configurable: true
                });
                ioService.prototype.registerSailsListener = function () {
                    var _this = this;
                    self["io"].socket.on('message', function (data) {
                        _this._ioMessage$.next(data.data);
                        //this.sendIOMessageBack(data.data);
                    });
                };
                ioService.prototype.subscribeToSails = function () {
                    self["io"].socket.get('/room/join');
                    this._ioMessage$.next({
                        message: "Joined Chat Room"
                    });
                };
                ioService.prototype.sendMessage = function (submission) {
                    self["io"].socket.get('/room/submit', {
                        message: submission["message"]
                    });
                };
                ioService.prototype.unsubscribeToSails = function () {
                    self["io"].socket.get('/room/leave');
                    this._ioMessage$.next({
                        message: "Left Chat Room"
                    });
                };
                ioService = __decorate([
                    core_1.Injectable(), 
                    __metadata('design:paramtypes', [])
                ], ioService);
                return ioService;
            }());
            exports_1("ioService", ioService);
        }
    }
});
//# sourceMappingURL=io.service.js.map